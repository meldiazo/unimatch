...
class MatchingController extends Controller
{
    ...
    private function creditMatch(BankStatementLine $line, PaymentVoucher $voucher, array $validated, User $user, ReconciliationSettings $settings): JsonResponse
    {
        if ($response = $this->ensurePending($line, $voucher)) {
            return $response;
        }
        ...
        if ($creditAmount > (float) $settings->credit_max_amount) {
            return response()->json([
        ...
        return response()->json(array_merge($payload, [
            ...
            'message' => 'Coincidencia confirmada y crédito registrado.',
        ]));
    }
    ...
    private function finalizeMatch(BankStatementLine $line, PaymentVoucher $voucher, User $user): array
    {
        ...
        $transaction = Transaction::create([
            ...
            'matched_by' => $user->id,
            'matched_at' => now(),
            'difference_amount' => $voucher->amount - $line->amount,
        ]);
